#!/bin/bash
# run_s2b_high_ani_final_slice.sh <slice_id> — syn2bani ani --calibrate for final high-ANI pairs.
# Output: $WORK/s2b_high_ani_final/{pairid}.tsv
set -uo pipefail
...
mkdir -p "$WORK/s2b_high_ani_final"

tail -n +2 "$PAIRS" | sed -n "${START},${END}p" | while IFS=$'\t' read -r PID QA RA _; do
    OUT="$WORK/s2b_high_ani_final/${PID}.tsv"
    [ -s "$OUT" ] && continue
    QF="$GENOMES/${QA}.fna"
    RF="$GENOMES/${RA}.fna"
    if [ ! -s "$QF" ] || [ ! -s "$RF" ]; then
...
echo "[high_ani_final s2b] slice $SLICE done"
