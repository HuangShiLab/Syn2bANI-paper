#!/usr/bin/env python3
"""Train GBRT v5 debiasing model with mash_ani feature.

Features: raw_ani, mash_ani, shared_log, af_q, af_r
...
def build_features(df: pd.DataFrame, reference_col: str = 'fastani_ani'):
    """Build feature matrix X and target y for GBRT v5."""
    ...
    meta_cols = ['query', 'reference', reference_col, 's2b_raw_ani', 's2b_mash_ani', 's2b_gbrt_ani']
...
def main():
    ...
    parser.add_argument('--output', default='results/gbrt_model_v5.json')
    parser.add_argument('--report', default='results/gbrt_v5_report.txt')
    ...
    meta['gbrt_v5_ani'] = (meta['s2b_raw_ani'] + meta['predicted_bias']).clip(0, 1)

    ref_col = 'fastani_ani'
    mask = meta[ref_col].notna() & meta['gbrt_v5_ani'].notna()
    sub = meta[mask]
    mae = mean_absolute_error(sub[ref_col], sub['gbrt_v5_ani'])
    rmse = np.sqrt(mean_squared_error(sub[ref_col], sub['gbrt_v5_ani']))
