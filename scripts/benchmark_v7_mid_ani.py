#!/usr/bin/env python3
"""Benchmark Syn2bANI v7 (TGT sparse chaining + chain k-mer ANI) vs skani on mid-ANI pairs."""
import argparse
import subprocess
import sys
...
def run_syn2bani(args: dict) -> dict:
            ...
            if len(parts) >= 14:
                try:
                    return {
                        ...
                        'chained_kmer_ani': float(parts[6]),
                        'gbrt_v7_ani': float(parts[13]),
                        'af_q': float(parts[7]),
                        'af_r': float(parts[8]),
                        'shared_tags': int(parts[9]),
                    }
    ...
    try:
        # Default output is chained-kmer ANI (mash_ani path with use_chained_kmer=true)
        cmd_default = [
            args['syn2bani'], 'dist',
        ...
        result['s2b_chained_kmer_ani'] = parsed_default['chained_kmer_ani']
        result['s2b_gbrt_v7_ani'] = parsed_gbrt['gbrt_v7_ani'] if parsed_gbrt else None
...
def main():
    ...
    tsv_path = output_dir / 'benchmark_v7_mid_ani.tsv'
    ...
    metrics = {
        ...
        'Syn2bANI mash_ani (v7)': compute_metrics(merged, 's2b_mash_ani'),
        'Syn2bANI chained_kmer_ani (v7 default)': compute_metrics(merged, 's2b_chained_kmer_ani'),
        'Syn2bANI calibrated (-0.028)': compute_metrics(merged, 's2b_calibrated'),
        'Syn2bANI GBRT v7': compute_metrics(merged, 's2b_gbrt_v7_ani'),
    }

    lines = []
    lines.append('# Syn2bANI v7 Mid-ANI Validation Report')
    lines.append('')
    ...
    detail_cols = ['query', 'reference', 'fastani_ani', 'skani_ani',
                   's2b_raw_ani', 's2b_mash_ani', 's2b_chained_kmer_ani',
                   's2b_calibrated', 's2b_gbrt_v7_ani',
    ...
    report_path = output_dir / 'benchmark_v7_mid_ani.md'
