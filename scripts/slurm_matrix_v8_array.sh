...
#SBATCH --time=8:00:00
#SBATCH --array=1-25
#SBATCH --output=logs/matrix_v8_%A_%a.out
#SBATCH --error=logs/matrix_v8_%A_%a.err
...
/usr/bin/time -v python3 scripts/run_benchmark_chunk_v8.py \
  --pairs "${CHUNK_FILE}" \
  --genomes "${GENOMES_DIR}" \
  --syn2bani "${SYN2BANI}" \
  --skani "${SKANI}" \
...
echo "Matrix v8 task ${SLURM_ARRAY_TASK_ID} complete"
