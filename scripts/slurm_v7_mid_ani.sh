#!/bin/bash -l
#SBATCH --job-name=s2b_v7_mid_ani
...
#SBATCH --mem=64G
#SBATCH --time=2:00:00
#SBATCH --output=logs/s2b_v7_mid_ani_%j.out
#SBATCH --error=logs/s2b_v7_mid_ani_%j.err
...
PAIRS_FILE="${VAL_DIR}/mid_ani_pairs_85_95.tsv"
...
OUTPUT_DIR="${PAPER_DIR}/results/validation/v7_mid_ani"

mkdir -p "${OUTPUT_DIR}" logs

echo "=== Step 1: Build Syn2bANI release ==="
cd "${SYN2BANI_DIR}"
cargo build --release

echo ""
echo "=== Step 2: Benchmark v7 (TGT sparse chaining + chain k-mer ANI) on mid-ANI pairs ==="
cd "${PAPER_DIR}"
python3 scripts/benchmark_v7_mid_ani.py \
  --pairs "${PAIRS_FILE}" \
  --genomes "${GENOMES_DIR}" \
